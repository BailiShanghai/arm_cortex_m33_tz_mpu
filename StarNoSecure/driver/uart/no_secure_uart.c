#include "no_secure_uart.h"
#include "stdio.h"
#include "string.h"
#include <stdbool.h>          // standard boolean definitions
#include <stdarg.h>

void no_secure_uart_init(uint32_t baud_rate, uint8_t rx_fifo_thr)
{
    ...
    REG_APB3_UART_CFG = 0;
    REG_APB3_UART_FIFO_THRESHOLD = 0;
...
int bk_no_secure_put_char(char *st)
{
    while (*st)
    {
...
static char _bk_no_secure_printf_buff[128];
int bk_no_secure_printf(const char *fmt, ...)
{
    va_list ap;
    int n;

    va_start(ap, fmt);
    n = vsnprintf(_bk_no_secure_printf_buff, sizeof(_bk_no_secure_printf_buff), fmt, ap);
    va_end(ap);
    bk_no_secure_put_char(_bk_no_secure_printf_buff);
